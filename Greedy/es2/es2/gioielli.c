...
Gioiello* Gioielli(const char* filename, float b, int *ret_size) {
	/*
		filename è il nome di un file di testo e b rappresenta il budget. La
		...
		senza frazionamenti. Un gioiello si può acquistare una sola volta. Si utilizzi
	...
	*/
	...
	for (unsigned int j = 0; j < numGioielli; j++)
		for (unsigned int k = j + 1; k < numGioielli; k++)
	...
	//greedy
	Gioiello * res = malloc(2 * sizeof(Gioiello)); // Allocazione iniziale
	i = 0;
	while (i<numGioielli) {
