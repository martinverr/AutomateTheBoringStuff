...
# Version 2:    I noticed that functions in versions 1 were not too long,
#               so I decided to join them in a single one
...
def isValidChessBoard(board):
    ...
    # Check pawns
    if list(board.values()).count("bpawn") > 8 or \
       list(board.values()).count("wpawn") > 8:
    ...
    # Check name of pieces
    for piece in list(board.values()):
        if not piece in allowedNames:
